...
/// The momentum phase of a scroll gesture.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum ScrollMomentumPhase {
    /// No momentum information is available.
    #[default]
    None,
    /// Momentum scrolling has just started.
    Started,
    /// Momentum scrolling is in progress.
    Moving,
    /// Momentum scrolling has ended.
    Ended,
    /// Momentum scrolling was cancelled.
    Cancelled,
}
...
pub struct ScrollWheelEvent {
    ...
    /// The momentum phase for inertial scrolling, when the platform provides it.
...
/// A smart magnify gesture event from the platform, generated by a discrete
/// zoom gesture such as a double-tap with two fingers on macOS.
#[derive(Clone, Debug, Default)]
...
impl Sealed for SmartMagnifyEvent {}
impl InputEvent for SmartMagnifyEvent {
    fn to_platform_input(self) -> PlatformInput {
        PlatformInput::SmartMagnify(self)
    }
}
impl GestureEvent for SmartMagnifyEvent {}
impl MouseEvent for SmartMagnifyEvent {}

impl Deref for SmartMagnifyEvent {
    type Target = Modifiers;
...
pub enum PlatformInput {
    ...
    Pinch(PinchEvent),
    /// A discrete smart magnify gesture was performed.
...
impl PlatformInput {
    pub(crate) fn mouse_event(&self) -> Option<&dyn Any> {
        match self {
            ...
            PlatformInput::SmartMagnify(event) => Some(event),
    ...
    pub(crate) fn keyboard_event(&self) -> Option<&dyn Any> {
        match self {
            ...
            PlatformInput::SmartMagnify(_) => None,
