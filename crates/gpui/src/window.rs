...
impl Window {
    ...
    /// Returns the native platform window identifier when available.
    pub fn native_window_id(&self) -> Option<u64> {
        self.platform_window.native_window_id()
    }

    /// Captures the current rendered scene to a PNG file when supported by the platform backend.
    pub fn capture_png(&self, path: &std::path::Path) -> Result<()> {
        self.platform_window
            .capture_png(&self.rendered_frame.scene, path)
    }
